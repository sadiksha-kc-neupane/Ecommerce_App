...
export default function Navbar() {
  ...
  const sellLink = (
    <Link
      to="/create-Product"
      onClick={() => setMobileOpen(false)}
      className="rounded-md border border-ochre px-3.5 py-1.5 font-mono text-xs uppercase tracking-widest text-ochre-ink transition hover:bg-ochre hover:text-navy"
    >
      Sell
    </Link>
  )
    ...
    <header className="sticky top-0 z-40 bg-white text-navy shadow-sm">
      ...
      <div className="bg-navy text-cream">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-1.5">
          ...
          <div className="hidden items-center gap-5 font-mono text-[11px] uppercase tracking-widest text-cream/70 sm:flex">
            {user?.role === "customer" && (
              ...
              <div className="mt-3 flex items-center justify-between border-t border-navy/10 pt-4 text-sm">
                ...
                {user ? (
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={handleLogout} className="font-medium text-rust/80 hover:text-rust">
                      Log out
                    </button>
                  </div>
                ) : (
